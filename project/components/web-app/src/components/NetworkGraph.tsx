...
import { neo4jApi } from "@/lib/neo4jApi";
...
export function NetworkGraph() {
  ...
  const handleApplicationSubmit = async (data: any) => {
    try {
      const transformedData = {
        ...
        ams_contacts_phone: data.ams_contacts_phone ? JSON.stringify(data.ams_contacts_phone.split(',').map(v => v.trim()).filter(Boolean)) : "[]",
  ...
  const handleSaveApplication = async (data: any) => {
    ...
    if (Object.keys(applicationData).length === 0) {
      neo4jApi.saveApplication(data).then((result) => {
      ...
      neo4jApi.editApplication(data).then((result) => {
        if (result && result.length > 0) {
  ...
  const handleSaveFlow = async (data: any) => {
    ...
    if (Object.keys(flowData).length === 0) {
      neo4jApi.saveFlow(data).then((result) => {
        if (result && result.length > 0) {
      ...
      neo4jApi.editFlow(data).then((result) => {
        if (result && result.length > 0) {
  ...
  const handleDeleteButton = async (data: any) => {
    ...
    if (type == "flow") {
      neo4jApi.deleteFlow(data).then((result) => {
        if (result) {
      ...
      neo4jApi.deleteApplication(data).then((result) => {
        if (result) {
  ...
  const expandNode = async (nodeId: string) => {
    ...
    try {
      ...
      const results = await neo4jApi.executeQuery(
        `
  ...
  useEffect(() => {
    neo4jApi.getConnectedApplicationLabels().then((result) => {
      if (result && result.length > 0) {
        setAppLabels(result);
      } else {
    ...
    neo4jApi.getFlowLabels().then((result) => {
      if (result && result.length > 0) {
  ...
  async function refreshGraph() {
    const results = await neo4jApi.executeQuery(query, {});
